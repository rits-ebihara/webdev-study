...
forEarch など Array のメソッドの場合、下記のようにしても1つ1つの非同期処理の完了を待たずに次の配列に移ります。
...
配列の順次処理の中に非同期処理を実行する場合は、2つの戦略を考えます。

1つは、1つづつ非同期処理を待ってシーケンシャルに処理する方法。

もう一つは、配列の数だけパラレル（並列）に処理して、すべての完了を待つ方法です。
