...
test('getCurrent - normal', () => {
  ...
  const result = ringBuffer.getCurrent();
  expect(result).toBe('test4');
});

test('getCurrent - at empty', () => {
...
test('currentIndex', () => {
  ...
  ringBuffer['_currentIndex'] = 4;
  expect(ringBuffer.currentIndex).toBe(4);
});

test('currentIndex - at empty', () => {
  const ringBuffer = new RingBuffer<string>(2);
  expect(ringBuffer.currentIndex).toBe(-1);
});
